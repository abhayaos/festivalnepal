...
export default function Footer() {
  return (
    <footer className="relative border-t border-orange-500/10 bg-gray-950/80">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="text-sm font-bold tracking-widest uppercase text-orange-400/80 mb-4">
                {title}
              </h4>
                ...
                {links.map((link) => (
                  <li key={link.label}>
                    {link.to ? (
                      <Link
                        to={link.to}
                        className="text-sm text-gray-500 hover:text-orange-300 transition-colors duration-300"
                      >
                        {link.label}
                      </Link>
                    ) : (
        ...
        {/* Bottom bar */}
        <div className="mt-12 pt-8 border-t border-orange-500/10 flex flex-col md:flex-row items-center justify-between gap-4">
